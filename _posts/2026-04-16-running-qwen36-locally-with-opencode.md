...
For context, Claude Sonnet 4.6 averages `~45 tk/s` on OpenRouter ([performance data](https://openrouter.ai/anthropic/claude-sonnet-4.6/performance)). Running a 35B model locally at `~49.5 tk/s` is on par with a top-tier cloud model.
...
I use LM Studio as the inference engine. The parameters below follow Unsloth's recommendations for Qwen3.6 thinking mode on precise coding tasks ([Unsloth Qwen3.6 Guide](https://unsloth.ai/docs/models/qwen3.6#llama-server-serving-and-openais-completion-library)).
...
The script lives at `~/.config/scripts/_gen-lmstudio-model.sh`. You can find it on GitHub Gist: [johnnyasantoss/_gen-lmstudio-model.sh](https://gist.github.com/johnnyasantoss/a5200046eca87dba2bafc8b670a4c946).
...
I plan to experiment with [LLamaBarn](https://github.com/ggml-org/LlamaBarn) next. But for now, [LM Studio](https://lmstudio.ai/) + [OpenCode](https://opencode.ai/) + [Qwen3.6](https://huggingface.co/unsloth/Qwen3.6-35B-A3B-GGUF) is a setup I am happy with.
